...
<div class="container-fluid">
    ...
    <div class="row justify-content-center py-4">
        <div class="col-md-4 col-lg-3">
            <div  class="position-sticky top-0 pt-2">
                <div class="d-flex align-items-center mb-2">
                    ...
                    <h1 class="fs-4 ms-3 my-0">Issue {{ $issue->edition }}</h1>
                </div>

                <p class="text-muted">{{ $issue->issue_date->format('l j F, Y') }}</p>

                <ul>
                    @foreach($posts as $feed)
                        <li>
                            <a href="#{{ $feed->first()->feed->id }}" class="text-decoration-none">{{ $feed->first()->feed_title ?? $feed->first()->feed->title }}</a>
                        </li>
                    @endforeach
                </ul>
                ...
                @auth
                    ...
                    @if(auth()->user()->hasDefaultDeliverySettings())
                        <div class="pb-4">
                            <div class="card">
                                <div class="card-body p-2 small text-center">
                                    <span class="fw-bold d-block">Did you know?</span>
                                    You can customise the days and time Subworthy delivers your daily email? <a href="{{ route('user.edit') }}#delivery" class="stretched-link">Click here...</a>
                                </div>
                            </div>
                        </div>
                    @endif
        ...
        <div class="col-md-8 col-lg-7">
            @foreach($posts as $feed)
            <div class="mb-4 pb-4">
            <h2 id="{{ $feed->first()->feed->id }}" class="h3 fw-bold pt-2">
                {{ $feed->first()->feed_title ?? $feed->first()->feed->title }}

                <a href="{{ $feed->first()->feed->link }}" target="_blank">
...
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
    ...
    <div class="offcanvas-body">
        ...
        <ul>
            @foreach($posts as $feed)
                <li>
                    <a href="#{{ $feed->first()->feed->id }}" class="text-decoration-none" onclick="hideOffCanvas()">{{ $feed->first()->feed->title }}</a>
                </li>
            @endforeach
        </ul>
    </div>
